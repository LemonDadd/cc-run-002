...
// GLSL smoothstep（GLSL ES 3.0/常见驱动在 edge0==edge1 时为未定义，键控参数下
// smoothness 可拖到 0，JS 侧做了阶跃兜底；测试中用非零 smooth 覆盖主路径）
...
test('B1 着色器 RGB 距离：权重 (1.2,1.0,1.2) 且整体 ×0.8（旧 CPU 是 (1.2,1.0,0.8) 且无尺度）', () => {
  assert.match(shadersSrc, /d \* vec3\(1\.2,\s*1\.0,\s*1\.2\)\) \* 0\.8/);
  // CPU 引擎不得再出现旧公式
  assert.doesNotMatch(cpuEngineSrc, /\(b - keyColor\[2\]\) \* 0\.8/);
  assert.ok(RGB_WEIGHTS[2] === 1.2 && RGB_SCALE === 0.8);
});

test('B2 三个模式的距离常量在 GLSL / 共享模块中一致', () => {
  assert.match(shadersSrc, /distance\(pu, ku\) \* 1\.8/);
  assert.match(shadersSrc, /dh \* 2\.2 \+ max\(hk\.y - h\.y, 0\.0\) \* 0\.25/);
  assert.equal(YUV_SCALE, 1.8);
  assert.equal(HSV_HUE_WEIGHT, 2.2);
  assert.equal(HSV_SAT_WEIGHT, 0.25);
});

test('B3 键控斜坡：两引擎都是 smoothstep（CPU 旧实现是线性 clamp）', () => {
  assert.match(shadersSrc, /smoothstep\(uThreshold - uSmooth, uThreshold \+ uSmooth, dist\)/);
  assert.match(cpuEngineSrc, /keyAlpha\(/);
  assert.doesNotMatch(cpuEngineSrc, /\(dist - \(t - sm\)\) \/ \(2 \* sm\)/);
});

test('B4 精修参数契约：feather*0.045、snap=0.12、semi=0.18/0.82、blend=0.55', () => {
  assert.match(glEngineSrc, /uFeatherW', params\.key\.feather \* 0\.045\)/);
  assert.match(shadersSrc, /uDenoise \* 0\.55/);
  assert.match(shadersSrc, /float snap = uDenoise \* 0\.12;/);
  assert.match(shadersSrc, /a > 0\.18 && a < 0\.82/);
