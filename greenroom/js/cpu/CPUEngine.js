...
export class CPUEngine {
  ...
  render(video, params, layout) {
    ...
    const featherW = params.key.feather * 0.045;
    ...
    // 统计
    if (this.frameNo % 30 === 0) this._updateStats(a, d, od, w, h);
  ...
  // 前背景光照/色彩统计：与 GLEngine._updateStats 同公式。
  // CPU 无法在合成前分离前景，这里用原始场景色按 alpha 加权近似 fg 预乘值，
  // 低 alpha 区域（合成图近似纯背景）近似背景色；输入 sceneRGB 为 0..255。
  _updateStats(alpha, scene, outData, w, h) {
    const n = w * h;
    let rS = 0, gS = 0, bS = 0, aS = 0;
    let brS = 0, bgS = 0, bbS = 0, bn = 0;
    for (let p = 0, i = 0; i < outData.length; i += 4, p++) {
      ...
      aS += aw;
      if (aw < 40 / 255) {
        brS += outData[i]; bgS += outData[i + 1]; bbS += outData[i + 2]; bn++;
      }
    }
    // GL 在 16×16 降采样上判定覆盖度，这里按比例换算到工作分辨率
    if (aS <= n * 12 / 256 || !bn) return;
    const fR = rS / aS, fG = gS / aS, fB = bS / aS;
    const bR = brS / bn, bG = bgS / bn, bB = bbS / bn;
    const targetExp = (0.299 * bR + 0.587 * bG + 0.114 * bB) /
                      (0.299 * fR + 0.587 * fG + 0.114 * fB + 1);
    const exp = Math.min(2, Math.max(0.5, targetExp));
