...
export class GLEngine {
  ...
  _postMask(src, dst, params) {
    ...
    p.set1('uFlipY', 0);
    p.set2('uPx', 1 / src.w, 1 / src.h);
    p.set1('uFeatherW', params.key.feather * KEY.FEATHER_SCALE); // 与 CPU 共用同一羽化系数
  ...
  _updateStats(fg, bgTex, params) {
    ...
    for (let i = 0; i < fgBuf.length; i += 4) {
      rS += fgBuf[i]; gS += fgBuf[i + 1]; bS += fgBuf[i + 2]; aS += fgBuf[i + 3];
    }
    // 背景
    let brS = 0, bgS2 = 0, bbS = 0;
    const bgBuf = read(1);
    for (let i = 0; i < bgBuf.length; i += 4) {
      brS += bgBuf[i]; bgS2 += bgBuf[i + 1]; bbS += bgBuf[i + 2];
    }
    const n = 256;
    if (aS > n * 12) {
      const fR = rS / aS, fG = gS / aS, fB = bS / aS;
      const bR = brS / n, bG = bgS2 / n, bB = bbS / n;
