...
// 色度键 / 遮罩精修的公式常量统一来自 js/keying.js —— CPU 回退引擎使用同一份
// 定义，保证 WebGL 与 CPU 在相同参数下公式逐项一致（见 tests/keying-parity.test.mjs）。
import { KEY } from '../keying.js';

// JS 数值 → GLSL 浮点字面量（1 → "1.0"）
const F = (x) => {
  const s = String(x);
...
float keyAlpha(vec3 c) {
  ...
  if (uMode == 1) {
    ...
    dist = distance(pu, ku) * ${F(KEY.YUV_SCALE)};
  } else if (uMode == 2) {
    // HSV 色相距离，叠加饱和度/亮度权重
    vec3 h = rgb2hsv(c);
    ...
    dh = min(dh, 1.0 - dh);
    dist = dh * ${F(KEY.HSV_DH)} + max(hk.y - h.y, 0.0) * ${F(KEY.HSV_DS)};
  } else {
    // RGB 加权色度距离
    vec3 d = c - uKeyColor;
    dist = length(d * ${V3(KEY.RGB_W)}) * ${F(KEY.RGB_SCALE)};
  }
  // dist 越小越像背景 → 背景 alpha 取反；sm 下限避免 smoothness=0 时 0/0
  float sm = max(uSmooth, ${F(KEY.SMOOTH_EPS)});
  float bg = smoothstep(uThreshold - sm, uThreshold + sm, dist);
  return clamp(bg, 0.0, 1.0);
}

// 边缘程度：alpha 梯度（用于边缘颜色校正）
float edgeFactor(float a) {
  return 1.0 - smoothstep(0.0, ${F(KEY.EDGE_HALF_WIDTH)}, abs(a - 0.5) * 2.0);
}
...
void main() {
  ...
  if (uSpill == 1) {
    ...
    // 边缘区域进一步去绿（与抠像边缘联动）
    wgt = clamp(wgt + edgeFactor(a) * uEdgeColor * ${F(KEY.SPILL_EDGE_MIX)}, 0.0, 1.0);
...
void main() {
  ...
  if (uDenoise > 0.001) {
    ...
    a = mix(a, mean, uDenoise * ${F(KEY.DENOISE_MIX)});
    float snap = uDenoise * ${F(KEY.DENOISE_SNAP)};
    bool isSemi = a > ${F(KEY.SEMI_LO)} && a < ${F(KEY.SEMI_HI)};
